...
class ProgressTracker:
    ...
    def progress_percent(self) -> float:
        ...
        Note: This property is not thread-safe and should only be called
        from the same async context that updates the progress.
        For thread-safe access, use get_progress_percent() instead.
        """
    ...
    def success_rate(self) -> float:
        ...
        Note: This property is not thread-safe and should only be called
        from the same async context that updates the progress.
        For thread-safe access, use get_success_rate() instead.
        """
        if self.completed == 0:
            return 0.0
    ...
    async def get_success_rate(self) -> float:
        """
        Get success rate in a thread-safe manner.
